...
// 		$consultaCol = 'INSERT INTO USUARIO (';
// 		$consultaVal = 'VALUES (';
// 		foreach (array_keys((array) new Usuario()) as $llave) {			
// 			$metodo = ucfirst($llave);
// 			$metodo = 'establecer'. $metodo;
// 			if (method_exists(Usuario, $metodo) && isset($usuario->{$llave})){
// 				$consultaCol = $consultaCol . preg_replace('/\B([A-Z])/', '_$1', $llave) . ',';
// 				$consultaVal = $consultaVal . "'" . $usuario->{$llave} . "'" . ',';
// 				$nusuario->$metodo($usuario->{$llave});
// 			} else {
// 				$this->cerrarConexion();
